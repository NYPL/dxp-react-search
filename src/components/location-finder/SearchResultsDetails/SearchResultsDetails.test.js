...
describe("SearchResultsDetails", () => {
  test("renders SearchResultsDetails component", () => {
    const { container } = render(<SearchResultsDetails />, {
      initialState: {
        search: {
    ...
    expect(container).toHaveTextContent(
      "Showing 92 results near 125th Library, by distance and open now. Clear all search terms."
    );
  });
  ...
  test("renders SearchResultsDetails open now only", () => {
    ...
    expect(container).toHaveTextContent(
      "Showing 50 results open now. Clear all search terms."
    );
  });
  ...
  test("renders SearchResultsDetails search query only", () => {
    ...
    expect(container).toHaveTextContent(
      "Showing 92 results near New York Botanical Garden, by distance. Clear all search terms."
    );
  });
  ...
  test("renders SearchResultsDetails search query + open now", () => {
    ...
    expect(container).toHaveTextContent(
      "Showing 50 results near Washington Square Park, by distance and open now. Clear all search terms."
    );
  });
