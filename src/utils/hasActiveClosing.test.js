...
describe("hasActiveClosing", () => {
  test("One active temporary closing should return true.", () => {
    ...
    expect(hasActiveClosing("2020-10-27T12:00:00-04:00", alerts, null)).toBe(
      true
    );
  });
  ...
  test("A single temporary closing with active hours and mins should return true.", () => {
    ...
    expect(hasActiveClosing("2020-10-27T12:05:00-04:00", alerts, null)).toBe(
      true
    );
  });
  ...
  test("A late opening should be considered an active closing.", () => {
    ...
    expect(hasActiveClosing("2020-10-28T12:00:00-04:00", alerts, "days")).toBe(
      true
    );
  });
  ...
  test("A early closing should be considered an active closing.", () => {
    ...
    expect(hasActiveClosing("2020-10-27T14:05:00-04:00", alerts, "days")).toBe(
      true
    );
  });
  ...
  test("A late opening that starts and ends before the current time should be active closing.", () => {
    const alerts = [
      {
        closed_for:
          "A late opening that starts and ends before the current time",
        applies: {
    ...
    expect(hasActiveClosing("2020-12-10T15:43:00-05:00", alerts, "days")).toBe(
      true
    );
  });
});
